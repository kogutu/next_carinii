...
export function ProductFiltersClient({
    ...
    const filters = useCategoryZustand(state => state.filters);
    ...
    useEffect(() => {
        ...
        if (_.isEmpty(selectedFilters)) return;
        setFilterZustand(selectedFilters)
        // const setZustandFilter
    }, [selectedFilters])
    useEffect(() => {
    ...
    return (
        <>
            ...
            {/* ════════════════════════════════════════════════════════════
                SORT DRAWER — shadcn Drawer (bottom sheet, snap to content)
            ════════════════════════════════════════════════════════════ */}
            ...
            {/* ════════════════════════════════════════════════════════════
                FILTER DRAWER — shadcn Drawer (full height, scrollable)
            ════════════════════════════════════════════════════════════ */}
...
// =====================================================================
// NUMERIC CHIP FACET — wartości numeryczne (1, 1.5, 4, 8) z suffix
// =====================================================================
                        ...
                        {visibleCounts.map(({ value, color }) => {
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => onToggle(value)}
                                    className={cn(
                                        'inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-3 text-xs font-medium transition-all',
                                        isSelected ? 'text-white shadow-sm' : 'text-gray-700'
                                    )}
                                    style={{
                                        background: isSelected ? '#000000' : color,
                                        backgroundColor: isSelected ? '#000000' : color, // backup
                                    }}
